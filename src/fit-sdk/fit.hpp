...
#define FIT_PROFILE_VERSION                                                                                            \
	((FIT_UINT16)(FIT_PROFILE_VERSION_MAJOR * FIT_PROFILE_VERSION_SCALE + FIT_PROFILE_VERSION_MINOR))
...
#define FIT_FLOAT32_INVALID                                                                                            \
	((FIT_FLOAT32) * ((FIT_FLOAT32 *)::fit::baseTypeInvalids[FIT_BASE_TYPE_FLOAT32 & FIT_BASE_TYPE_NUM_MASK]))
...
#define FIT_FLOAT64_INVALID                                                                                            \
	((FIT_FLOAT64) * ((FIT_FLOAT64 *)::fit::baseTypeInvalids[FIT_BASE_TYPE_FLOAT64 & FIT_BASE_TYPE_NUM_MASK]))
...
#define FIT_MESSAGE_INDEX_FIELD_NUM                                                                                    \
	FIT_FIELD_NUM_MESSAGE_INDEX // For reverse compatibility only.  Use FIT_FIELD_NUM_MESSAGE_INDEX instead.
#define FIT_TIMESTAMP_FIELD_NUM                                                                                        \
	FIT_FIELD_NUM_TIMESTAMP // For reverse compatibility only.  Use FIT_FIELD_NUM_TIMESTAMP instead.
...
#define FIT_STRUCT_OFFSET(MEMBER, STRUCT_TYPE)                                                                         \
	(((FIT_UINT8 *)&(((STRUCT_TYPE *)FIT_NULL)->MEMBER)) -                                                             \
	 ((FIT_UINT8 *)(FIT_NULL))) // Computes the byte offset of a member in a file.  Compiles to a constant.

// Offset of message in file, not including file header.
#define FIT_MESG_OFFSET(MESG_MEMBER, MESG_INDEX, MESG_SIZE, FILE)                                                      \
	(FIT_STRUCT_OFFSET(MESG_MEMBER, FILE) + MESG_INDEX * (FIT_HDR_SIZE + MESG_SIZE) +                                  \
	 FIT_HDR_SIZE) // Computes the byte offset of a message in a file structure.  Compiles to a constant.
#define FIT_MESG_DEF_OFFSET(MESG_DEF_MEMBER, FILE)                                                                     \
	(FIT_STRUCT_OFFSET(MESG_DEF_MEMBER, FILE) +                                                                        \
	 FIT_HDR_SIZE) // Computes the byte offset of a message definition in a file structure.  Compiles to a constant.
#define FIT_MESG_DEF_FIELD_OFFSET(FIELD_MEMBER, FIELD_INDEX)                                                           \
	(FIT_STRUCT_OFFSET(FIELD_MEMBER, FIT_FIELD_DEF) +                                                                  \
	 FIT_FIELD_DEF_SIZE *                                                                                              \
	     FIELD_INDEX) // Computes the byte offset of a field definition member.  Compiles to a constant.

// Offset of message in file including file header.
#define FIT_FILE_MESG_OFFSET(MESG_MEMBER, MESG_INDEX, MESG_SIZE, FILE)                                                 \
	(FIT_FILE_HDR_SIZE + FIT_MESG_OFFSET(MESG_MEMBER, MESG_INDEX, MESG_SIZE, FILE))
...
typedef std::wstring
    FIT_WSTRING; // Returned by message/field "Get" APIs and required by "Set" APIs. Encoding defined in fit_config.hpp.
