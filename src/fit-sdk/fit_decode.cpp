...
Decode::Decode() : mesgListener(NULL), mesgDefinitionListener(NULL) {
...
FIT_BOOL Decode::Read(std::istream *file, MesgListener *mesgListener, MesgDefinitionListener *definitionListener,
			...
			case RETURN_MESG:
				if (mesg.GetNum() == FIT_MESG_NUM_DEVELOPER_DATA_ID) {
					...
					if (!descMesg.IsFieldDefinitionNumberValid()) {
						throw fit::RuntimeException(
						    "Invalid developer field definition number in FieldDescriptionMesg");
					}
...
Decode::RETURN Decode::ReadByte(FIT_UINT8 data) {
	if ((fileBytesLeft > 0) && (skipHeader == FIT_FALSE)) {
		...
		if (fileBytesLeft == 1) // CRC low byte.
		{
			if (state != STATE_RECORD) {
				std::ostringstream message;
				message << "FIT decode error: Decoder not in correct state after last data byte in file. Check message "
				           "definitions. Error at byte: "
				        << currentByteOffset;
				throw(RuntimeException(message.str()));
			}
	...
	case STATE_FILE_HDR:
		...
		case 0:
			if (data < FIT_HEADER_SIZE_NO_CRC) {
				std::ostringstream message;
				message << "FIT decode error: File header size invalid.  File is not FIT. Error at byte : "
				        << currentByteOffset;
			...
			break;
		case 1:
			if ((data & FIT_PROTOCOL_VERSION_MAJOR_MASK) >
			    (FIT_PROTOCOL_VERSION_MAJOR << FIT_PROTOCOL_VERSION_MAJOR_SHIFT)) {
				std::ostringstream message;
				message << "FIT decode error: Protocol version " << (data & FIT_PROTOCOL_VERSION_MAJOR_MASK)
				        << FIT_PROTOCOL_VERSION_MAJOR_SHIFT << "." << (data & FIT_PROTOCOL_VERSION_MINOR_MASK)
				        << " not supported.  Must be " << FIT_PROTOCOL_VERSION_MAJOR << ".15 or earlier.";
				headerException = message.str();
			}
			break;
		case 4:
		...
		case 8:
			if (data != '.') {
				std::ostringstream message;
				message << "FIT decode error: File header signature mismatch.  File is not FIT. Error at byte : "
				        << currentByteOffset;
				headerException = message.str();
			}
			break;
		...
		if (fileBytesLeft > 1) {
			if ((data & FIT_HDR_TIME_REC_BIT) != 0) {
				...
				if (localMesgDefs[localMesgIndex].GetNum() == FIT_MESG_NUM_INVALID) {
					std::ostringstream message;
					message << "FIT decode error: Missing FIT message definition for local message number "
					        << ((int)localMesgIndex) << ". Error at byte: " << currentByteOffset;
					throw(RuntimeException(message.str()));
				}
	...
	case STATE_MESG_NUM_1:
		...
		if (archs[localMesgIndex] == FIT_ARCH_ENDIAN_BIG) {
			localMesgDefs[localMesgIndex].SetNum((localMesgDefs[localMesgIndex].GetNum() >> 8) |
			                                     ((localMesgDefs[localMesgIndex].GetNum() & 0xFF) << 8));
		} else if (archs[localMesgIndex] != FIT_ARCH_ENDIAN_LITTLE) {
			std::ostringstream message;
			message << "FIT decode error: Architecture " << archs[localMesgIndex]
			        << " not supported. Error at byte: " << currentByteOffset;
			throw(RuntimeException(message.str()));
		}
	...
	case STATE_DEV_FIELD_INDEX:
		...
		try {
			const FieldDescriptionMesg &desc =
			    descriptions.at(fieldData[DevFieldIndexOffset]).at(fieldData[DevFieldNumOffset]);
			const DeveloperDataIdMesg &developer = developers.at(fieldData[DevFieldIndexOffset]);

			localMesgDefs[localMesgIndex].AddDevField(
			    DeveloperFieldDefinition(desc, developer, fieldData[DevFieldSizeOffset]));
		} catch (std::out_of_range) {
			// No Matching Description Message add a Generic Definition
			localMesgDefs[localMesgIndex].AddDevField(DeveloperFieldDefinition(
			    fieldData[DevFieldNumOffset], fieldData[DevFieldSizeOffset], fieldData[DevFieldIndexOffset]));
		}
							...
							for (j = 0; j < mesg.GetNumFields(); j++) {
								...
								for (k = 0; k < numComponents; k++) {
									...
									if ((fc->num == field.GetNum()) && (fc->accumulate)) {
										value = ((((value / field.GetScale()) - field.GetOffset()) + fc->offset) *
										         fc->scale);
									}
								}
							}
				...
				if (!suppressComponentExpansion) {
					if (activeSubField == FIT_SUBFIELD_INDEX_MAIN_FIELD) {
						if (mesg.GetFieldByIndex(i)->GetNumComponents() > 0) {
							ExpandComponents(mesg.GetFieldByIndex(i), mesg.GetFieldByIndex(i)->GetComponent(0),
							                 mesg.GetFieldByIndex(i)->GetNumComponents());
						}
					} else {
						if (mesg.GetFieldByIndex(i)->GetSubField(activeSubField)->numComponents > 0) {
							ExpandComponents(mesg.GetFieldByIndex(i),
							                 mesg.GetFieldByIndex(i)->GetSubField(activeSubField)->components,
							                 mesg.GetFieldByIndex(i)->GetSubField(activeSubField)->numComponents);
						}
					}
				}
			}
	...
	case STATE_DEV_FIELD_DATA: {
		...
		if (fieldBytesLeft == 0) {
			...
			if (fieldBytesLeft == 0) {
				fieldBytesLeft = localMesgDefs->GetDevFieldByIndex(++fieldIndex)->GetSize();
			}
		}
...
void Decode::ExpandComponents(Field *containingField, const Profile::FIELD_COMPONENT *components,
	...
	for (i = 0; i < numComponents; i++) {
		...
		if (component->num != FIT_FIELD_NUM_INVALID) {
			...
			if (componentField.GetNumComponents() == 1) {
				if (componentField.IsSignedInteger())
					value = (((signedBitsValue / (FIT_FLOAT64)component->scale) - component->offset) +
					         componentField.GetComponent(0)->offset) *
					        componentField.GetComponent(0)->scale;
				else
					value = (((bitsValue / (FIT_FLOAT64)component->scale) - component->offset) +
					         componentField.GetComponent(0)->offset) *
					        componentField.GetComponent(0)->scale;
			...
			// The component field is itself a composite field (more than one component).  Don't use scale/offset,
			// containing field data must already be encoded.  Add elements to it until we have added bitsvalue
			...
			// componentField is an ordinary field, apply scale and offset as usual
				...
				if (componentField.IsSignedInteger())
					value = (((signedBitsValue / (FIT_FLOAT64)component->scale) - component->offset) +
					         componentField.GetOffset(subfieldIndex)) *
					        componentField.GetScale(subfieldIndex);
				else
					value = (((bitsValue / (FIT_FLOAT64)component->scale) - component->offset) +
					         componentField.GetOffset(subfieldIndex)) *
					        componentField.GetScale(subfieldIndex);
