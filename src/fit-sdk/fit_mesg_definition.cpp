...
MesgDefinition::MesgDefinition(void) : num(FIT_MESG_NUM_INVALID), localNum(0), fields(), devFields() {
}

MesgDefinition::MesgDefinition(const Mesg &mesg) : num(mesg.GetNum()), localNum(mesg.GetLocalNum()) {
...
FIT_BOOL MesgDefinition::Supports(const MesgDefinition &mesgDef) const {
	...
	for (int i = 0; i < (int)mesgDef.fields.size(); i++) {
		...
		if (mesgDef.fields[i].GetSize() >
		    supportedFieldDef->GetSize()) // Other field definition is larger than this field definition.
			return FIT_FALSE;
	}
...
int MesgDefinition::Write(std::ostream &file) const {
	...
	if (devFields.size() > 0) {
		file.put((FIT_HDR_TYPE_DEF_BIT | FIT_HDR_DEV_FIELD_BIT) |
		         (localNum & FIT_HDR_TYPE_MASK)); // Message definition record header with local message number.
	} else {
		file.put((FIT_HDR_TYPE_DEF_BIT) |
		         (localNum & FIT_HDR_TYPE_MASK)); // Message definition record header with local message number.
	}
	file.put(0); // Reserved
	file.put(GetArch());
	if (GetArch()) { // Big Endian
