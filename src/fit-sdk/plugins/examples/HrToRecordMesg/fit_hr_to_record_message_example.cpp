...
public:
	...
	void OnMesg(fit::HrMesg &mesg) {
		...
		if ((mesg.GetNumFilteredBpm() != FIT_UINT8_INVALID) && (mesg.GetNumFilteredBpm() > 0)) {
		...
		if ((mesg.GetNumEventTimestamp12() != FIT_UINT8_INVALID) && (mesg.GetNumEventTimestamp12() > 0)) {
		...
		if ((mesg.GetNumEventTimestamp() != FIT_UINT8_INVALID) && (mesg.GetNumEventTimestamp() > 0)) {
...
#pragma warning(suppress : 4996) // warning C4996: 'fopen': This function or variable may be unsafe. Consider using
                                 // fopen_s instead.
			fileDescriptor = fopen(outputfile.c_str(), "w");
			// Write the csv header out
			fprintf(fileDescriptor,
			        "Type,Local Number,Message,Field 1,Value 1,Units 1,Field 2,Value 2,Units 2,Field 3,Value 3,Units "
			        "3,Field 4,Value 4,Units 4,Field 5,Value 5,Units 5,Field 6,Value 6,Units 6\n");
		}
	}
